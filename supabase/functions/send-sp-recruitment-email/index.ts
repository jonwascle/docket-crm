// This runs on Supabase's servers, never in the browser.
//
// Called from the "Info & pricing email" stage on a service provider's
// Recruitment tab. Sends them a one-off email introducing Wascle, with
// their pricing PDF attached and their onboarding link included directly
// in the message.
//
// Same email provider as the other email functions — SMTP2GO. Needs the
// same two secrets already set up project-wide:
//   SMTP2GO_API_KEY
//   SP_EMAIL_FROM (a no-reply address, since suppliers never reply to this)

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const LOGO_JPEG_BASE64 = '';

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
};

Deno.serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  try {
    const supabaseUrl = Deno.env.get('SUPABASE_URL');
    const serviceRoleKey = Deno.env.get('SB_SECRET_KEY');
    const adminClient = createClient(supabaseUrl, serviceRoleKey);

    const authHeader = req.headers.get('Authorization') || '';
    const token = authHeader.replace('Bearer ', '').trim();
    if (!token) {
      return json({ error: 'Missing authorization header.' }, 401);
    }
    const { data: { user }, error: userErr } = await adminClient.auth.getUser(token);
    if (userErr || !user) {
      return json({ error: 'Not signed in.' }, 401);
    }

    const body = await req.json().catch(() => ({}));
    const { spEmail, spName, onboardingLink, pdfBase64 } = body;
    if (!spEmail || !onboardingLink || !pdfBase64) {
      return json({ error: 'Missing required fields.' }, 400);
    }

    const html = buildEmailHtml(spName, onboardingLink);

    const safeName = (spName || 'service-provider').replace(/[^a-z0-9]+/gi, '-');
    const result = await sendEmail([spEmail], 'Wascle — working together as a service provider', html, [
      { filename: `${safeName}-pricing.pdf`, content: pdfBase64 }
    ]);

    if (result.error) {
      return json({ error: result.error }, 400);
    }
    return json({ success: true }, 200);
  } catch (e) {
    return json({ error: e.message || 'Unknown error.' }, 500);
  }
});

// Comfortaa is loaded via Google Fonts for clients that support it (Gmail
// web, Apple Mail); everywhere else this quietly falls back to a plain
// sans-serif, since not all email clients (Outlook especially) render
// custom web fonts.
function buildEmailHtml(spName: string, onboardingLink: string) {
  const greetingName = escapeHtmlServer(spName || 'there');
  return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
</head>
<body style="margin:0;padding:0;">
  <div style="font-family: Arial, sans-serif; background-color: #F7F5F0; padding: 32px 16px;">
    <div style="max-width: 560px; margin: 0 auto; background-color: #FFFFFF; border-radius: 12px; overflow: hidden; border: 1px solid #E5E1D8; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
      <div style="background-color: #FFFFFF; padding: 26px 32px; border-bottom: 1px solid #F0EBDD;">
        <img src="data:image/jpeg;base64,${LOGO_JPEG_BASE64}" alt="Wascle" width="160" style="display:block;">
      </div>
      <div style="padding: 36px 32px 8px; font-family: Arial, sans-serif; font-size: 14.5px; line-height: 1.65; color: #1B1B1B;">
        <p style="margin: 0 0 16px;">Hi ${greetingName},</p>
        <p style="margin: 0 0 16px;">Thank you for taking the time to speak with us today. As discussed, we'd be delighted to welcome you on board as one of our service providers.</p>
        <div style="border-left: 3px solid #F5B429; background-color: #FDF6E7; border-radius: 0 6px 6px 0; padding: 14px 18px; margin: 0 0 20px;">
          <p style="margin: 0;">Please find attached the pricing for the services we discussed. Have a look through them and let us know if you have any questions.</p>
        </div>
        <p style="margin: 0 0 16px;">To get your account set up, please complete our secure onboarding form using the link below. It should only take a few minutes and covers your company details, team members, waste transfer station, and key documents such as your Waste Carrier's Licence and insurance.</p>
        <p style="margin: 0 0 24px;text-align:center;">
          <a href="${escapeHtmlServer(onboardingLink)}" style="display: inline-block; background-color: #1B1B1B; color: #F5B429; text-decoration: none; padding: 13px 26px; border-radius: 6px; font-weight: 700; font-size: 14px;">Complete onboarding form →</a>
        </p>
        <p style="margin: 0 0 16px;">If you don't have everything to hand, don't worry. You can save your progress at any time and return later to complete the form. Any information you enter is automatically saved to our system, so completing as much as you can now will help us get you onboarded and receiving work as quickly as possible.</p>
        <p style="margin: 0 0 16px;">If you have any questions during the onboarding process, please don't hesitate to get in touch — we're always happy to help.</p>
        <p style="margin: 0 0 28px;">We look forward to working with you.</p>
        <p style="margin: 0 0 32px;border-top:1px solid #F0EBDD;padding-top:20px;">Kind regards,<br><b style="color:#B8860B;">The Wascle Team</b></p>
      </div>
      <div style="height: 4px; background: linear-gradient(90deg, #F5B429 0%, #f0a51e 100%);"></div>
      <div style="background-color: #F7F5F0; padding: 16px 32px; font-family: Arial, sans-serif; font-size: 12px; color: #7A7568;">
        <b style="color:#1B1B1B;">Wascle</b> &middot; <a href="https://wascle.co.uk" style="color: #B8860B;font-weight:600;">wascle.co.uk</a>
      </div>
    </div>
  </div>
</body>
</html>
  `;
}

async function sendEmail(to: string[], subject: string, html: string, attachments: { filename: string, content: string }[]) {
  const apiKey = Deno.env.get('SMTP2GO_API_KEY');
  const fromAddress = Deno.env.get('SP_EMAIL_FROM');
  if (!apiKey || !fromAddress) {
    return { error: 'Email service is not configured yet (missing SMTP2GO_API_KEY or SP_EMAIL_FROM).' };
  }
  const res = await fetch('https://api.smtp2go.com/v3/email/send', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      api_key: apiKey,
      sender: fromAddress,
      to,
      subject,
      html_body: html,
      attachments: attachments.map(a => ({
        filename: a.filename,
        fileblob: a.content,
        mimetype: 'application/pdf',
      })),
    }),
  });
  const data = await res.json().catch(() => null);
  if (!res.ok || !data || data.data?.succeeded !== 1) {
    return { error: (data && JSON.stringify(data)) || await res.text() };
  }
  return { success: true };
}

function escapeHtmlServer(str: string) {
  return String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

function json(body: unknown, status: number) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}