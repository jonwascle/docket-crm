// This runs on Supabase's servers, never in the browser.
//
// Two things it does:
//   'notify_admins'    - a new bug report or feature request has just
//                        been submitted. Emails every admin with the
//                        full details.
//   'notify_released'  - an admin has just marked something as
//                        released. Emails every single Dockit user
//                        (not just admins) with what changed.
//
// Needs the same two secrets as the other internal emails
// (Supabase -> Edge Functions -> feature-request-notify -> Settings -> Secrets):
//   SMTP2GO_API_KEY
//   TASK_EMAIL_FROM

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const LOGO_JPEG_BASE64 = '';

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
};

Deno.serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  try {
    const supabaseUrl = Deno.env.get('SUPABASE_URL');
    const serviceRoleKey = Deno.env.get('SB_SECRET_KEY');
    const adminClient = createClient(supabaseUrl, serviceRoleKey);

    const body = await req.json().catch(() => ({}));
    const action = body.action;

    // This one action is deliberately public — it's what powers the
    // "watch the demo" link in release announcement emails, and those
    // get opened by people who aren't necessarily signed into Dockit.
    if (action === 'get_public_release') {
      const { requestId } = body;
      console.log('DEBUG get_public_release requestId:', requestId);
      if (!requestId) return json({ error: 'Missing requestId.' }, 400);

      const { data: reqRow, error: reqErr } = await adminClient
        .from('feature_requests').select('title, type, release_notes, release_video_path, release_video_transcript, released_at, status')
        .eq('id', requestId).single();
      console.log('DEBUG reqRow (no status filter):', JSON.stringify(reqRow));
      console.log('DEBUG reqErr:', reqErr ? JSON.stringify(reqErr) : 'none');
      if (reqErr || !reqRow) return json({ error: 'Not found.' }, 404);
      if (reqRow.status !== 'released') return json({ error: 'Not released yet.' }, 404);

      let videoUrl = null;
      if (reqRow.release_video_path) {
        const { data: signedData } = await adminClient.storage
          .from('feature-request-media').createSignedUrl(reqRow.release_video_path, 3600);
        videoUrl = signedData ? signedData.signedUrl : null;
      }

      return json({
        title: reqRow.title,
        type: reqRow.type,
        release_notes: reqRow.release_notes,
        release_video_transcript: reqRow.release_video_transcript,
        released_at: reqRow.released_at,
        videoUrl
      }, 200);
    }

    const authHeader = req.headers.get('Authorization') || '';
    const token = authHeader.replace('Bearer ', '').trim();
    if (!token) return json({ error: 'Missing authorization header.' }, 401);
    const { data: { user } } = await adminClient.auth.getUser(token);
    if (!user) return json({ error: 'Not signed in.' }, 401);

    if (action === 'notify_admins') {
      const { requestId } = body;
      if (!requestId) return json({ error: 'Missing requestId.' }, 400);

      const { data: reqRow, error: reqErr } = await adminClient
        .from('feature_requests').select('*').eq('id', requestId).single();
      if (reqErr || !reqRow) return json({ error: 'Request not found.' }, 404);

      const { data: submitter } = await adminClient.from('profiles').select('name, email').eq('id', reqRow.submitted_by).maybeSingle();

      const { data: admins } = await adminClient.from('profiles').select('email').eq('role', 'admin');
      const toAddresses = (admins || []).map(a => a.email).filter(Boolean);
      if (toAddresses.length === 0) return json({ success: true, skipped: true }, 200);

      const html = buildAdminNotifyEmailHtml(reqRow, submitter);
      const typeLabel = reqRow.type === 'bug' ? 'Bug report' : 'Feature request';
      await sendEmail(toAddresses, `${typeLabel}: ${reqRow.title}`, html);

      return json({ success: true }, 200);
    }

    if (action === 'notify_released') {
      const { requestId, recipientIds } = body;
      if (!requestId) return json({ error: 'Missing requestId.' }, 400);

      const { data: reqRow, error: reqErr } = await adminClient
        .from('feature_requests').select('*').eq('id', requestId).single();
      if (reqErr || !reqRow) return json({ error: 'Request not found.' }, 404);

      let profilesQuery = adminClient.from('profiles').select('email, name');
      if (Array.isArray(recipientIds) && recipientIds.length > 0) {
        profilesQuery = profilesQuery.in('id', recipientIds);
      }
      const { data: allProfiles, error: profilesErr } = await profilesQuery;
      if (profilesErr) return json({ error: profilesErr.message }, 500);
      const recipients = (allProfiles || []).filter(p => p.email);
      if (recipients.length === 0) return json({ success: true, skipped: true, sent: 0 }, 200);

      const typeLabel = reqRow.type === 'bug' ? 'Bug fix' : 'New feature';
      let sent = 0;
      for (const p of recipients) {
        try {
          const html = buildReleasedEmailHtml(reqRow, p.name);
          const result = await sendEmail([p.email], `${typeLabel} in Dockit: ${reqRow.title}`, html);
          if (!result.error) sent++;
        } catch (e) { /* keep going for everyone else */ }
      }

      return json({ success: true, sent }, 200);
    }

    return json({ error: 'Unknown action.' }, 400);
  } catch (e) {
    return json({ error: e.message || 'Unknown error.' }, 500);
  }
});

function buildAdminNotifyEmailHtml(reqRow, submitter) {
  const typeLabel = reqRow.type === 'bug' ? 'Bug report' : 'Feature request';
  const submitterName = escapeHtmlServer(submitter ? (submitter.name || submitter.email) : 'A user');
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;">
  <div style="font-family: Arial, sans-serif; background-color: #F7F5F0; padding: 32px 16px;">
    <div style="max-width: 560px; margin: 0 auto; background-color: #FFFFFF; border-radius: 12px; overflow: hidden; border: 1px solid #E5E1D8; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
      <div style="background-color: #FFFFFF; padding: 26px 32px; border-bottom: 1px solid #F0EBDD;">
        <img src="data:image/jpeg;base64,${LOGO_JPEG_BASE64}" alt="Wascle" width="160" style="display:block;">
      </div>
      <div style="padding: 36px 32px 8px; font-family: Arial, sans-serif; font-size: 14.5px; line-height: 1.65; color: #1B1B1B;">
        <p style="margin: 0 0 16px;">New ${typeLabel.toLowerCase()} submitted in Dockit.</p>
        <div style="border-left: 3px solid #F5B429; background-color: #FDF6E7; border-radius: 0 6px 6px 0; padding: 14px 18px; margin: 0 0 20px;">
          <p style="margin: 0 0 8px;"><b>${escapeHtmlServer(reqRow.title)}</b></p>
          <p style="margin: 0;white-space:pre-wrap;">${escapeHtmlServer(reqRow.details || 'No further details given.')}</p>
        </div>
        <p style="margin: 0 0 16px;color:#7A7568;">Submitted by ${submitterName} on ${new Date(reqRow.submitted_at).toLocaleDateString('en-GB')}.</p>
        <p style="margin: 0 0 32px;">See the Feedback section in Dockit for the full tracker, including a copy-friendly version of this request.</p>
      </div>
      <div style="height: 4px; background: linear-gradient(90deg, #F5B429 0%, #f0a51e 100%);"></div>
    </div>
  </div>
</body></html>
  `;
}

function buildReleasedEmailHtml(reqRow, recipientName) {
  const isFeature = reqRow.type !== 'bug';
  const typeLabel = isFeature ? 'new feature' : 'bug fix';
  const typeLabelCap = isFeature ? 'New feature' : 'Bug fixed';
  const typeIcon = isFeature ? '✨' : '🔧';
  const accentColor = isFeature ? '#F5B429' : '#3A7D44';
  const accentBg = isFeature ? '#FDF6E7' : '#EAF4EB';
  const accentText = isFeature ? '#946200' : '#2C6135';
  const greetingName = escapeHtmlServer((recipientName || '').split(' ')[0] || 'there');

  const transcriptSection = reqRow.release_video_transcript
    ? `<div style="margin: 24px 0 0;">
         <p style="font-size:11px;font-weight:700;color:#9A9488;text-transform:uppercase;letter-spacing:0.6px;margin:0 0 10px;">From the video</p>
         <div style="border-left: 3px solid #E5E1D8; padding: 4px 0 4px 16px;">
           <p style="margin: 0;white-space:pre-wrap;color:#55504a;font-size:13.5px;font-style:italic;line-height:1.7;">${escapeHtmlServer(reqRow.release_video_transcript)}</p>
         </div>
       </div>`
    : '';
  const watchLinkSection = reqRow.release_video_path
    ? `<p style="margin: 28px 0 12px;">Watch the short video below explaining this ${typeLabel}:</p>
       <p style="margin: 0;text-align:center;">
         <a href="https://dockit-wascle.vercel.app/#watch-demo=${reqRow.id}" style="display: inline-block; background-color: #1B1B1B; color: #ffffff; text-decoration: none; padding: 14px 28px; border-radius: 8px; font-weight: 700; font-size: 14px; letter-spacing:0.2px;">▶&nbsp;&nbsp;Watch the video</a>
       </p>`
    : '';

  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background-color:#EFEBE2;">
  <div style="font-family: Arial, sans-serif; padding: 40px 16px;">
    <div style="max-width: 560px; margin: 0 auto; background-color: #FFFFFF; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 28px rgba(27,27,27,0.08);">
      <div style="background-color: #FFFFFF; padding: 30px 32px 20px; border-bottom: 1px solid #F0EBDD;">
        <img src="data:image/jpeg;base64,${LOGO_JPEG_BASE64}" alt="Wascle" width="150" style="display:block;">
      </div>
      <div style="padding: 32px 36px 8px; font-family: Arial, sans-serif; color: #1B1B1B;">
        <p style="margin: 0 0 20px;font-size:14.5px;">Hi ${greetingName},</p>
        <div style="display:inline-block;background-color:${accentBg};color:${accentText};font-size:11.5px;font-weight:700;letter-spacing:0.4px;text-transform:uppercase;padding:5px 12px;border-radius:20px;margin-bottom:16px;">${typeIcon}&nbsp; ${typeLabelCap}</div>
        <p style="margin: 0 0 4px;font-size:14.5px;">We've deployed a ${typeLabel}:</p>
        <h1 style="font-size:22px;line-height:1.3;margin:0 0 20px;color:#1B1B1B;">${escapeHtmlServer(reqRow.title)}</h1>
        <p style="margin: 0 0 8px;font-size:13px;font-weight:700;color:#9A9488;text-transform:uppercase;letter-spacing:0.4px;">Here's a quick summary of the ${typeLabel}</p>
        <p style="margin: 0;white-space:pre-wrap;font-size:14.5px;line-height:1.7;color:#3A3A3A;">${escapeHtmlServer(reqRow.release_notes || '')}</p>
        ${transcriptSection}
        ${watchLinkSection}
        <div style="border-top:1px solid #EFEBE2;margin:36px 0 0;padding-top:20px;">
          <p style="margin: 0 0 28px;color:#9A9488;font-size:12.5px;">Curious what else is in the pipeline, or want to suggest something yourself? Head to the <b style="color:#55504a;">Feedback</b> section in Dockit any time.</p>
        </div>
      </div>
      <div style="height: 5px; background: linear-gradient(90deg, ${accentColor} 0%, #f0a51e 100%);"></div>
    </div>
    <p style="text-align:center;color:#B0A99A;font-size:11.5px;margin:20px 0 0;">This is an automatic notification from Dockit.</p>
  </div>
</body></html>
  `;
}

async function sendEmail(to: string[], subject: string, html: string) {
  const apiKey = Deno.env.get('SMTP2GO_API_KEY');
  const fromAddress = Deno.env.get('TASK_EMAIL_FROM');
  if (!apiKey || !fromAddress) {
    return { error: 'Email service is not configured yet (missing SMTP2GO_API_KEY or TASK_EMAIL_FROM).' };
  }
  const res = await fetch('https://api.smtp2go.com/v3/email/send', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ api_key: apiKey, sender: fromAddress, to, subject, html_body: html }),
  });
  const data = await res.json().catch(() => null);
  if (!res.ok || !data || data.data?.succeeded !== 1) {
    return { error: (data && JSON.stringify(data)) || await res.text() };
  }
  return { success: true };
}

function escapeHtmlServer(str: string) {
  return String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

function json(body: unknown, status: number) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}