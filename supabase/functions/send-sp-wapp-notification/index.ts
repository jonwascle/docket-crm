...
// (Supabase -> Edge Functions -> send-sp-wapp-notification -> Settings -> Secrets):
//   SMTP2GO_API_KEY
//   TASK_EMAIL_FROM
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const html = buildWappNotificationEmailHtml(spName);

    const result = await sendEmail(toAddresses, `New service provider set up: ${spName || 'Unnamed'}`, html, [
      { filename: `${safeName}-wapp-setup.pdf`, content: pdfBase64 }
    ]);
...
function buildWappNotificationEmailHtml(spName: string) {
  const safeSpName = escapeHtmlServer(spName || 'A new service provider');
  return `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
